...
class ImageViewer(QMainWindow):
    def __init__(self):
        ...
        self.setWindowTitle("Simple Manga Viewer")
    ...
    def display_image(self):
        if self.images:
            ...
            if self.is_denoise_on:
                if self.is_sharpen_on:
                    print("Anime4k mode")
                    processed_pixmap = self.anime4k(image_path)
                else:
                    print("Only denoise is on")
                    processed_pixmap = self.denoise(image_path)
            else:
                if self.is_sharpen_on:
                    print("Only sharpen is on")
                    processed_pixmap = self.sharpen(image_path)
                else:
                    print("Neither denoise nor sharpen is on")
                    processed_pixmap = pixmap

            scaled_pixmap = processed_pixmap.scaled(self.size(), Qt.KeepAspectRatio, self.scaling_method)
    ...
    def denoise(self, path):
        img = cv2.imread(path)
        dst = cv2.fastNlMeansDenoisingColored(img, None, 10, 10, 7, 21)

        height, width, channel = dst.shape
        bytesPerLine = 3 * width
        qImg = QImage(dst.data, width, height, bytesPerLine, QImage.Format_RGB888).rgbSwapped()

        return QPixmap.fromImage(QImage(qImg))

    def sharpen(self, path):
        img = cv2.imread(path)
        spi = cv2.detailEnhance(img)

        height, width, channel = spi.shape
        bytesPerLine = 3 * width
        qImg = QImage(spi.data, width, height, bytesPerLine, QImage.Format_RGB888).rgbSwapped()
    ...
    def anime4k(self, path):
        ...
        return self.pil2pixmap(new_img)
...
class SettingsDialog(QDialog):
    def __init__(self, parent=None):
        ...
        self.radio_nlmd = QRadioButton("Denoise (SuperSlow)")
        self.radio_sharp = QRadioButton("Sharpen (SuperSlow)")
        self.radio_both = QRadioButton("Anime4k (Fast)")
        self.layout.addWidget(self.radio_linear)
        self.layout.addWidget(self.radio_smooth)
        self.layout.addWidget(self.radio_nlmd)
        self.layout.addWidget(self.radio_sharp)
        self.layout.addWidget(self.radio_both)
    ...
    def get_selected_method(self):
        ...
        if self.radio_nlmd.isChecked():
            return 1
        if self.radio_sharp.isChecked():
            return 2
